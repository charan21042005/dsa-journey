...
### 3️⃣ Using `#define` Preprocessor Directive
...
👉 Best practice: Prefer `const` or `constexpr` over `#define`.
...
int area = PI * 5 * 5;
```
✅ The second approach is clearer, safer, and reusable.
...
| Rule                                   | Explanation         |
| -------------------------------------- | ------------------- |
| Must be initialized                    | `auto x;` ❌ invalid |
...
* `cerr` → un-buffered error output stream
...
### Characteristics of `cerr`
| Feature       | Description     |
| ------------- | --------------- |
| Purpose       | Error messages  |
| Buffered      | ❌ No            |
| Output timing | Immediate       |
| Reliability   | High for errors |

### When to Use `cerr`
* Debugging
* Displaying critical error messages
* When output must appear immediately

---

## 🔹 Buffered Standard Error Stream `(clog)`
`clog` is also used for error or logging messages, but it is buffered.
...
| ------------ | --------- | ------- |
| Buffered     | No        | Yes     |
| Output speed | Immediate | Delayed |
| Use case     | Errors    | Logs    |
| Performance  | Slower    | Faster  |

---

## 🔹 C++ Escape Sequences
Escape sequences are **special characters** used within strings
to represent actions like newline, tab, or quotation marks.

They begin with a **backslash** `(\)`.

![C++ Escape Sequences Table](../Images/4..png)
...
Example 3
```cpp
cout << "She said, \"C++ is powerful\"";
```
Output:
```cpp
She said, "C++ is powerful"
```
...
int main() {
    ...
    if (age < 0) {
        ...
        clog << "Log: Age entered successfully\n";
...
### ✅ Summary
* `<iostream>` provides standard I/O streams
* `cout` is used for output using <<
* `cin` is used for input using >>
* `cerr` prints un-buffered error messages
* `clog` prints buffered log messages
* Escape sequences control formatting inside strings

Understanding I/O is **essential for user interaction, debugging, and logging**
in C++ programs and is heavily used in **DSA problem solving**.

---


📘 **Next in this series:**  
Continue with operators, conditions, and loops in  
[`2.operators_and_loops.md`](./2.operators_and_loops.md)
